...
struct Bar : View {
    ...
    var body: some View {
        ...
        .frame(width: getSize(weight: Int(goldAsset.weight)).width, height: getSize(weight: Int(goldAsset.weight)).height)
        ...
        .overlay {
            RoundedRectangle(cornerRadius: CGFloat(radius), style: .circular)
...
struct Bar_Previews: PreviewProvider {
    static var previews: some View {
        Bar(width: 60, height: 100, goldAsset: Gold.init(type: .bar, unit: .grams, weight: 10, dateAdded: Date.now))
    }
}
